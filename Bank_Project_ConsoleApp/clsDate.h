...
private:
    ...
    static short ReadYear(string Message = "\nEnter a Year? ")
    {
        short Year;
        cout << Message;
        cin >> Year;
        ...
        while (cin.fail() || cin.peek() != '\n')
        {
            ...
            cout << "Invalid Year, Enter a valid one:" << endl;
            cin >> Year;
        }
    ...
    static short ReadMonth(string Message = "\nEnter a Month? ", short From = 1, short To = 12)
    {
        short Month;
        cout << Message;
        cin >> Month;

        while ((!(Month >= From && Month <= To)) || cin.fail() || cin.peek() != '\n')
        {
            cin.clear();
            cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
    ...
    static short ReadDay(short Year, short Month, string Message = "\nEnter a Day? ")
    {
        switch (NumberOfDaysInAMonth(Year, Month))
        {
        case 28:
            return ReadValidateShortInRange(Message, 1, 28);
        case 29:
            return ReadValidateShortInRange(Message, 1, 29);
        case 30:
            return ReadValidateShortInRange(Message, 1, 30);
        default:
            return ReadValidateShortInRange(Message, 1, 31);
        }
    }
...
public:
    ...
    {
        return DayShortName(DayOfWeekOrder(Date)) + " , " + to_string(Date._Day) + "/" + to_string(Date._Month) + "/" + to_string(Date._Year);
    }
    ...
    {
        string arrDayNames[] = {"Sunday", "Monday", "Tuesday", "Wednesday", "Thuesday", "Friday", "Saturday"};
        return arrDayNames[DayOfWeekOrder];
    }
    ...
    {
        return DaysFromTheBeginingOfTheYear(Date._Year, Date._Month, Date._Day);
    }
    ...
    bool IsThisDateAfterDate2(clsDate Date2)
    {
    ...
    clsDate ReadFullDate(clsDate Date)
    {
        ...
        Date._Day = ReadDay(Date._Year, Date._Month);
        return Date;
    }
    ...
    static string GetLocalDateString()
    {
        ...
        char *dt = ctime(&t); // convert in string form (array of chars)
        return dt;
    }

    static string GetUTCDateString()
    {
        time_t t = time(0);       // get time now
        tm *gmtm = gmtime(&t);    // converting now to tm struct for UTC date/time
        char *dt = asctime(gmtm); // convert from structure to string (array of chars)
